...
/** WhatsApp image messages only accept JPEG/PNG links — not WebP (portal/Bunny uploads). */
...
async function resolveWhatsAppImagePayload({
  phoneNumberId,
  accessToken,
  imageUrl,
}: {
  ...
  if (isDirectWhatsAppImageLink(imageUrl) && !needsWhatsAppImageConversion(imageUrl)) {
