...
export function InboxPanel() {
  ...
  async function fetchConversations() {
    ...
    setLoading(false);
    if (data.conversations?.length && !selectedId) {
    ...
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
        <p className="text-base font-medium text-slate-800">
          No conversations need human handoff.
        </p>
        <p className="mt-2 text-sm text-slate-600">
          Escalated WhatsApp chats will appear here.
        </p>
      </div>
    );
  }
        ...
        {conversations.map((conv) => (
          <button
            ...
            className={`block w-full border-b border-slate-200 px-4 py-3 text-left transition-colors hover:bg-white ${
              selectedId === conv.id ? "border-l-4 border-l-blue-600 bg-white" : "border-l-4 border-l-transparent"
            }`}
          >
            <p className="text-sm font-semibold text-slate-900">
              +{conv.customer_phone}
            </p>
            <p className="text-xs font-medium text-slate-600">{conv.status}</p>
          </button>
        ))}
      </div>
